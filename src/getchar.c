...
gotchars(char_u *chars, int len)
{
  ...
  while (todo--)
  {
    ...
    if (buflen == 2)
      continue;
    if (buflen == 3 && buf[1] == KS_EXTRA && (buf[2] == KE_FOCUSGAINED || buf[2] == KE_FOCUSLOST))
    {
      ...
      for (;;)
      {
        ...
        else if (typebuf.tb_len > 0)
        {
          ...
          if (no_mapping == 0 && maphash_valid && (no_zero_mapping == 0 || c1 != '0') && (typebuf.tb_maplen == 0 || (p_remap && (typebuf.tb_noremap[typebuf.tb_off] & (RM_NONE | RM_ABBR)) == 0)) && !(p_paste && (State & (INSERT + CMDLINE))) && !(State == HITRETURN && (c1 == CAR || c1 == ' ')) && State != ASKMORE && State != CONFIRM)
          {
            ...
            {
              LANGMAP_ADJUST(c1,
                             (State & (CMDLINE | INSERT)) == 0 && get_real_state() != SELECTMODE);
              nolmaplen = 0;
            }
#endif
#ifdef FEAT_LOCALMAP
...
#ifdef FEAT_EVAL
                  STRNCMP(s, save_m_keys != NULL ? save_m_keys : mp->m_keys,
                          (size_t)keylen)
#else
    ...
    buf_T *buf) /* buffer for local mappings or NULL */
{
      ...
      for (; mp; mp = mp->m_next)
      {
        ...
        do /* do this twice if c2 is set, 3 times with c3 */
        {
          ...
          if (!did_cpo)
          {
            ...
            if (did_cpo)
            {
              if (fprintf(fd, "let s:cpo_save=&cpo") < 0 || put_eol(fd) < 0 || fprintf(fd, "set cpo&vim") < 0 || put_eol(fd) < 0)
                return FAIL;
            }
          }
          if (c1 && putc(c1, fd) < 0)
  ...
  if (did_cpo)
    if (fprintf(fd, "let &cpo=s:cpo_save") < 0 || put_eol(fd) < 0 || fprintf(fd, "unlet s:cpo_save") < 0 || put_eol(fd) < 0)
      return FAIL;
  return OK;
}
...
#if defined(MSWIN) || defined(MACOS_X) || defined(PROTO)
/*
