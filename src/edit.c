...
executionStatus_T state_edit_execute(void *ctx, int c)
{
  ...
  case Ctrl_H:
    context->did_backspace =
        ins_bs(c, BACKSPACE_CHAR, &context->inserted_space);
    auto_format(FALSE, TRUE);
    break;
...
#endif
    context->did_backspace =
        ins_bs(c, BACKSPACE_WORD, &context->inserted_space);
    auto_format(FALSE, TRUE);
    break;

  case Ctrl_U: /* delete all inserted text in current line */
    context->did_backspace =
        ins_bs(c, BACKSPACE_LINE, &context->inserted_space);
    auto_format(FALSE, TRUE);
    context->inserted_space = FALSE;
    break;
         ...
         long count)
{
  ...
  for (;;)
  {
     ...
     */
    if (curbuf->b_mod_set && curwin->w_p_wrap && !did_backspace &&
    ...
    )
    {
      ...
      if (
#ifdef FEAT_VARTABS
          (int)curwin->w_wcol < mincol - tabstop_at(get_nolist_virtcol(),
                                                    curbuf->b_p_ts,
...
int ins_need_undo_get(void) { return ins_need_undo; }
 ...
 */
static void insert_special(int c, int allow_modmask,
 ...
 */
void insertchar(int c,             /* character to insert or NUL */
                            ...
                            int c) /* character to be inserted (can be NUL) */
{
  ...
  while (!got_int)
  {
    ...
    while ((!fo_ins_blank && !has_format_option(FO_INS_VI)) ||
           (flags & INSCHAR_FORMAT) || curwin->w_cursor.lnum != Insstart.lnum ||
 ...
 * saved here.
 */
void auto_format(int trailblank, /* when TRUE also format with trailing blank */
 ...
 * position.
 */
static void check_auto_format(int end_insert) /* TRUE when ending Insert mode */
{
  int c = ' ';
  int cc;
 ...
 */
int comp_textwidth(int ff) /* force formatting (for "gq" command) */
{
  int textwidth;
 ...
 */
void start_arrow(pos_T *end_insert_pos) /* can be NULL */
{
...
static void start_arrow_common(pos_T *end_insert_pos, /* can be NULL */
                               int end_change)        /* end undoable change */
{
 ...
 */
static void stop_insert(pos_T *end_insert_pos,
                        ...
                        int nomove) /* <c-\><c-o>, don't move cursor */
{
  ...
  if (did_restart_edit == 0 ||
      (ptr != NULL && (int)STRLEN(ptr) > new_insert_skip))
  {
  ...
  if (!arrow_used && end_insert_pos != NULL)
  {
    ...
    if (!ins_need_undo && has_format_option(FO_AUTO))
    {
      ...
      if (VIM_ISWHITE(cc))
      {
        ...
        // the "coladd".
        if (gchar_cursor() == NUL && curwin->w_cursor.lnum == tpos.lnum &&
...
void beginline(int flags)
{
  ...
  else
  {
    ...
    if (flags & (BL_WHITE | BL_SOL))
    {
      ...
      for (ptr = ml_get_curline();
           VIM_ISWHITE(*ptr) && !((flags & BL_FIX) && ptr[1] == NUL); ++ptr)
        ++curwin->w_cursor.col;
    }
    curwin->w_set_curswant = TRUE;
  }
...
int cursor_up(long n, int upd_topline) /* When TRUE: update topline */
{
  linenr_T lnum;
 ...
 */
int cursor_down(long n, int upd_topline) /* When TRUE: update topline */
{
  linenr_T lnum;
...
void replace_push(int c) /* character that is replaced (NUL is none) */
{
  char_u *p;
 ...
 * encountered.
 */
static void replace_join(int off) /* offset for which NUL to remove */
{
  int i;
 ...
 * insert.
 */
static int ins_esc(long *count, int cmdchar, int nomove) /* don't move cursor */
{
  int temp;
...
static int ins_bs(int c, int mode, int *inserted_space_p)
{
  ...
  else
  {
     ...
     */
    if (mode == BACKSPACE_CHAR &&
        ((p_sta && in_indent) || ((get_sts_value() != 0
#ifdef FEAT_VARTABS
                                    ...
                                    (!*inserted_space_p || arrow_used))))))
    {
      ...
      *inserted_space_p = FALSE;
      ...
      if (p_sta && in_indent)
      {
        ...
        want_vcol =
            tabstop_start(want_vcol, get_sts_value(), curbuf->b_p_vsts_array);
#else
      if (p_sta && in_indent)
        ts = (int)get_sw_value(curbuf);
      else
...
static void ins_up(int startcol) /* when TRUE move to Insstart.col */
{
  pos_T tpos;
...
static void ins_down(int startcol) /* when TRUE move to Insstart.col */
{
  pos_T tpos;
  linenr_T old_topline = curwin->w_topline;
#ifdef FEAT_DIFF
...
static int ins_tab(void)
{
...
#endif
  {
    ...
    if ((State & REPLACE_FLAG) && fpos.lnum == Insstart.lnum &&
